...
import { appConfig, persistSession, readWalletAddress, readSignerSecret, readSignerPublicKey, readCredentialId, bytesToHex, deriveFeePayerKeypair, requirePasskeyAssertion } from './lib';
    ...
    return (<section class="panel hero-panel">
      ...
      <div class="stack">
        ...
        <button class="primary" onClick={handleRegister} disabled={!canStart()}>
          <Show when={wallet.isPending() || status() === 'registering'} fallback={<Show when={status() === 'deploying'} fallback="Create wallet">
              Deploying wallet...
            </Show>}>
            Creating passkey...
          </Show>
...
function DashboardPage() {
    ...
    const [signature, setSignature] = createSignal(null);
    const [error, setError] = createSignal(null);
    onMount(() => {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const tx = new TransactionBuilder(account, {
                ...
                .addOperation(contract.call('transfer', nativeToScVal(feePayer.publicKey(), { type: 'address' }), nativeToScVal(recipient(), { type: 'address' }), nativeToScVal(amountStroops, { type: 'i128' })))
                .setTimeout(30)
                .build();
