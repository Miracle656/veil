 ...
 */
export function browserPrfEvaluator(credentialId, rpId) {
    return async (salt) => {
...
export async function deriveKeyFromPrf(prfOutput) {
    const baseKey = await globalThis.crypto.subtle.importKey('raw', toArrayBuffer(prfOutput), 'HKDF', false, ['deriveKey']);
    return globalThis.crypto.subtle.deriveKey({ name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: HKDF_INFO }, baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
}
/** Import a raw 32-byte key as an AES-GCM-256 key (used for the fallback path). */
async function importRawAesKey(raw) {
    return globalThis.crypto.subtle.importKey('raw', toArrayBuffer(raw), { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
}
...
export async function encryptWithKey(key, plaintext) {
    ...
    const ct = new Uint8Array(await globalThis.crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, toArrayBuffer(data)));
...
async function loadOrCreateFallbackKey(storage) {
 ...
 */
export async function createLocalCipher(config) {
    ...
    if (prfOutput && prfOutput.length > 0) {
        ...
        if (!storage) {
            throw new Error('WebAuthn PRF is unavailable and no storage adapter was provided for a fallback key.');
        }
