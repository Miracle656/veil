...
export function createWallet(config: WalletConfig | string) {
    ...
    return {
        ...
        send: async (
            ...
            const { networkPassphrase, rpcUrl } = (core as any).config;
            ...
            (core as any).updateState({ isPending: true, error: null });
            ...
            try {
                ...
                const amountStroops = BigInt(Math.round(parseFloat(amount.toString()) * 10_000_000));
                ...
                const tx = new TransactionBuilder(sourceAccount, {
                    ...
                    .addOperation(
                        sacContract.call(
                            'transfer',
                            nativeToScVal(smartWalletAddress, { type: 'address' }),
                ...
                (core as any).updateState({ isPending: false, error: null });
                return sendResult.hash;

            } catch (err: unknown) {
                const message = err instanceof Error ? err.message : String(err);
                store.update(s => ({ ...s, status: 'error', error: message }));
                (core as any).updateState({ isPending: false, error: message });
                throw err;
            }
        }
    };
