...
jest.mock('@stellar/stellar-sdk', () => ({
    ...
    Account: jest.fn().mockImplementation((_id, seq) => ({
        accountId: () => _id,
        sequenceNumber: () => seq,
...
Object.defineProperty(global, 'crypto', {
    value: {
        getRandomValues: jest.fn((arr) => (arr.fill(42), arr)),
    },
    writable: true,
    configurable: true,
});
...
describe('useInvisibleWallet', () => {
    ...
    describe('register()', () => {
        ...
        it('throws when navigator.credentials is unavailable (no WebAuthn support)', async () => {
            const savedCredentials = global.navigator.credentials;
    ...
    describe('login()', () => {
        ...
        it('restores the session and marks the wallet deployed when the contract exists on-chain', async () => {
            ...
            jest.mocked(SorobanRpc.Server).mockImplementation(() => ({ getContractData: jest.fn().mockResolvedValue({}) }));
            const { result } = renderHook(() => useInvisibleWallet(CONFIG));
            let loginResult;
            await act(async () => { loginResult = await result.current.login(); });
    ...
    describe('signAuthEntry()', () => {
        ...
        it('throws when the payload is not 32 bytes', async () => {
            ...
            await expect(act(async () => { await result.current.signAuthEntry(badPayload); })).rejects.toThrow('32 bytes');
        });
        it('throws when no key ID is stored (not yet registered)', async () => {
            ...
            await expect(act(async () => { await result.current.signAuthEntry(payload); })).rejects.toThrow(/No key ID/);
        });
    });
    // ── initial state ──────────────────────────────────────────────────────────
