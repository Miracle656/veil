...
const mockToXDR = jest.fn().mockReturnValue('mock-xdr-string');
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const mockOperationPathPayment = jest.fn();
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const mockOperationChangeTrust = jest.fn();
...
jest.unstable_mockModule('@stellar/stellar-sdk', () => {
    ...
    function AssetConstructor(code, issuer) {
        return { isNative: () => false, getCode: () => code, getIssuer: () => issuer };
    }
    AssetConstructor.native = () => nativeAsset;
    return {
        Horizon: {
...
function makeAccount(balances = [{ asset_type: 'native', balance: '100.0000000' }]) {
...
describe('buildPayment', () => {
    ...
    it('returns an XDR string', async () => {
        const input = { to_address: 'GDEST', asset: 'XLM', amount: 10, wallet_address: 'GSOURCE' };
        expect(await buildPayment(input)).toBe('mock-xdr-string');
    });
    it('passes the correct destination to Operation.payment', async () => {
        const input = { to_address: 'GDEST123', asset: 'XLM', amount: 5, wallet_address: 'GSOURCE' };
        await buildPayment(input);
        expect(mockOperationPayment).toHaveBeenCalledWith(expect.objectContaining({ destination: 'GDEST123' }));
    });
    it('formats amount to 7 decimal places', async () => {
        const input = { to_address: 'GDEST', asset: 'XLM', amount: 1.5, wallet_address: 'GSOURCE' };
        await buildPayment(input);
        expect(mockOperationPayment).toHaveBeenCalledWith(expect.objectContaining({ amount: '1.5000000' }));
    });
    it('adds text memo when memo is provided', async () => {
        const input = { to_address: 'GDEST', asset: 'XLM', amount: 1, wallet_address: 'GSOURCE', memo: 'hello' };
        await buildPayment(input);
        expect(mockAddMemo).toHaveBeenCalledWith(expect.objectContaining({ type: 'text', value: 'hello' }));
    });
    it('does not add memo when memo is absent', async () => {
        const input = { to_address: 'GDEST', asset: 'XLM', amount: 1, wallet_address: 'GSOURCE' };
        await buildPayment(input);
    ...
    it('loads the source account via wallet_address', async () => {
        const input = { to_address: 'GDEST', asset: 'XLM', amount: 1, wallet_address: 'GMYWALLET' };
        await buildPayment(input);
    ...
    it('throws when Horizon rejects (invalid wallet_address)', async () => {
        ...
        const input = { to_address: 'GDEST', asset: 'XLM', amount: 1, wallet_address: 'GINVALID' };
...
describe('buildSwap', () => {
    ...
    it('returns an XDR string', async () => {
        const input = { from_asset: 'XLM', to_asset: 'XLM', amount: 10, wallet_address: 'GSOURCE' };
        expect(await buildSwap(input)).toBe('mock-xdr-string');
    });
    it('adds a pathPaymentStrictSend operation', async () => {
        const input = { from_asset: 'XLM', to_asset: 'XLM', amount: 5, wallet_address: 'GSOURCE' };
        await buildSwap(input);
        expect(mockOperationPathPayment).toHaveBeenCalled();
    });
    it('formats sendAmount to 7 decimal places', async () => {
        const input = { from_asset: 'XLM', to_asset: 'XLM', amount: 3.14, wallet_address: 'GSOURCE' };
        await buildSwap(input);
        expect(mockOperationPathPayment).toHaveBeenCalledWith(expect.objectContaining({ sendAmount: '3.1400000' }));
    });
    it('defaults destMin to amount * 0.995 when min_received is absent', async () => {
        const input = { from_asset: 'XLM', to_asset: 'XLM', amount: 100, wallet_address: 'GSOURCE' };
        await buildSwap(input);
        expect(mockOperationPathPayment).toHaveBeenCalledWith(expect.objectContaining({ destMin: (100 * 0.995).toFixed(7) }));
    });
    it('uses explicit min_received as destMin', async () => {
        const input = { from_asset: 'XLM', to_asset: 'XLM', amount: 100, min_received: 95, wallet_address: 'GSOURCE' };
        await buildSwap(input);
        expect(mockOperationPathPayment).toHaveBeenCalledWith(expect.objectContaining({ destMin: '95.0000000' }));
    });
    it('prepends changeTrust when destination asset trustline is missing', async () => {
        ...
        const input = { from_asset: 'XLM', to_asset: 'USDC:GUSDC_ISSUER', amount: 10, wallet_address: 'GSOURCE' };
        await buildSwap(input);
    ...
    it('sets a 180-second timeout', async () => {
        const input = { from_asset: 'XLM', to_asset: 'XLM', amount: 1, wallet_address: 'GSOURCE' };
        await buildSwap(input);
        expect(mockSetTimeout).toHaveBeenCalledWith(180);
    });
    it('throws for malformed asset string (missing issuer)', async () => {
        const input = { from_asset: 'BADTOKEN', to_asset: 'XLM', amount: 1, wallet_address: 'GSOURCE' };
        await expect(buildSwap(input)).rejects.toThrow(/"BADTOKEN" must be "CODE:ISSUER"/);
    });
    it('throws when Horizon rejects (invalid wallet_address)', async () => {
        mockLoadAccount.mockRejectedValueOnce(new Error('Account not found'));
        const input = { from_asset: 'XLM', to_asset: 'XLM', amount: 1, wallet_address: 'GINVALID' };
