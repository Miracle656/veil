...
export type PasskeyLoginOptions = {
  ...
  /** Hex-encoded 65-byte passkey public key supplied by the same recovery source. */
  passkeyPublicKeyHex?: string;
  /** Test/integration override; otherwise a new fee payer is generated for no-PRF recovery. */
 ...
 */
export async function loginWithPasskey(options: PasskeyLoginOptions = {}): Promise<LoginResult> {
  ...
  if (sdkAddress && sdkKeyId) {
    ...
    if (prf && prf.length >= 32) {
      ...
      const pkBytes =
        sdkPubKey && /^[0-9a-fA-F]{130}$/.test(sdkPubKey)
          ? new Uint8Array(Buffer.from(sdkPubKey, 'hex'))
          : null;
  ...
  if (!picked.prf || picked.prf.length < 32) {
    const walletAddress = options.walletAddress ?? sdkAddress;
    const publicKeyHex = options.passkeyPublicKeyHex ?? sdkPubKey;
    if (!walletAddress || !publicKeyHex) {
      throw new Error(
        "This passkey didn't return a PRF secret. Recover with the wallet address and passkey public key from an encrypted backup, or use the recovery-server flow."
      );
    }
    if (!/^[0-9a-fA-F]{130}$/.test(publicKeyHex)) {
      throw new Error('The recovered passkey public key is invalid.');
    }

    // A no-PRF device cannot recreate the original fee payer. A fresh one is
    // still valid for __check_auth because the passkey signs the auth entry;
    // it simply needs XLM before the first transaction can be submitted.
    const feePayer = options.feePayerSecret
      ? Keypair.fromSecret(options.feePayerSecret)
      : Keypair.random();
    await Promise.all([
      ...
      AsyncStorage.multiSet([
        [SDK_ADDRESS, walletAddress],
        [SDK_KEY_ID, picked.credentialId],
        [SDK_PUBLIC_KEY, publicKeyHex],
      ]),
    ]);
    return { address: walletAddress, source: 'recovered' };
  }
