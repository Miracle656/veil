...
| E-4 | CI runner compromise exposes `NEXT_PUBLIC_*` env vars | GitHub Actions | `NEXT_PUBLIC_*` values are public network addresses — testnet endpoints and the public mainnet factory ID — browser-exposed by design. The paid mainnet RPC key is held in a **server-only** `MAINNET_RPC_URL` (never `NEXT_PUBLIC_`), so it is not among the CI-exposed values | ✅ No mainnet secrets in `NEXT_PUBLIC_*` |
