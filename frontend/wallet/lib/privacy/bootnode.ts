 ...
 * Veil runs its own at `NEXT_PUBLIC_SPP_BOOTNODE_URL`, but it may be:
...
import { NETHERMIND_BOOTNODE_URL } from './config.js';
...
export async function resolveBootnodeWithFallback(
  ...
  if (!primary) {
    const entry: CacheEntry = {
      url: fallback,
      usingFallback: true,
      reason: 'No SPP bootnode configured (NEXT_PUBLIC_SPP_BOOTNODE_URL is unset). Using Nethermind\'s public bootnode.',
      resolvedAt: Date.now(),
    };
