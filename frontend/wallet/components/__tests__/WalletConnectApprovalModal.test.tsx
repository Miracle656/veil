...
jest.mock('@/lib/network', () => ({ getNetwork: () => ({ networkPassphrase: 'test' }) }));
jest.mock('@/lib/passkeyAuth', () => ({ requirePasskey: jest.fn() }));
jest.mock('@/lib/walletConnect', () => ({
  getWalletConnectClient: jest.fn(),
  getWalletConnectSessions: jest.fn(() => []),
  handleSignXdrRequest: jest.fn(),
}));
...
const request = (xdr = 'AAAA') => ({ params: { request: { params: { xdr } } } });
